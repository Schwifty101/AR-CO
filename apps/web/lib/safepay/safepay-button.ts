...
// @sfpy/checkout-components is a UMD bundle that Turbopack cannot
// statically analyze for ESM exports. Use require() to bypass.
// eslint-disable-next-line @typescript-eslint/no-require-imports
const safepay = require('@sfpy/checkout-components') as
 ...
 * - `client`: Object with environment keys mapping to public API keys
 * - `style`: Optional button styling configuration
 * - `orderId`: Optional reference ID for the order
 * - `source`: Optional source identifier for analytics
 * - `payment`: Payment details (currency and amount in PKR major unit)
 * - `onPayment`: Callback fired on successful payment
 * - `onCancel`: Callback fired when user cancels payment
 *
