...
## Status: COMPLETE — All batches done, code reviewed, critical fixes applied
...
### Batch 5: Code Review Fixes ✅

- Fixed C1: Added `sanitizePostgrestFilter()` to complaints service for `.ilike()` filter
- Fixed C2: Changed `{ attorneyProfileId: staffId }` to `{ staffId }` in frontend `assignRegistration`
- Fixed M5: Added `clientProfileId` guard before `as string` cast in complaints + service-registrations
- Fixed L1: Changed `replace('_', ' ')` to `replace(/_/g, ' ')` in subscription status display
- Added `BadRequestException` import to complaints service
- Updated init.md checkboxes (HEAD TASK 6)
- **110 tests passing**, type checks pass for both api and web
